...
def get_block_limits_and_order(is_weekend):
    if is_weekend:
        return (
            ...
            ["03.05R", "03.05QA", "03.05QB"]  # DAY -> LATE EVE -> NIGHT
        )
    else:
        return (
            {
...
def billing_summary(entries, total_hours, is_weekend):
    ...
    for block in BLOCK_ORDER:
        ...
        cap = BLOCK_UNIT_LIMITS.get(block, 0)
...
with st.expander("🧮 Calculate (No Optimization)", expanded=False):
    if st.button("Calculate Billing"):
        ...
        st.session_state.BLOCK_UNIT_LIMITS, BLOCK_ORDER = get_block_limits_and_order(is_weekend)        
...
with st.expander("⚙️ Efficiency Optimizer", expanded=False):
    ...
    if run_opt:
        ...
        if is_weekend:
            FILLER_BLOCK_ORDER = ["03.05R", "03.05QA"]  # Only valid on weekends/stats
        else:
            FILLER_BLOCK_ORDER = ["03.05N", "03.05P", "03.05QA"]  # Only valid on weekdays  # A (day), B (eve), C (late eve)
        ...
        fillers = []
        BLOCK_UNIT_LIMITS, BLOCK_ORDER = get_block_limits_and_order(is_weekend)
        st.write("BLOCK_UNIT_LIMITS:", BLOCK_UNIT_LIMITS)
        st.write("BLOCK_ORDER:", BLOCK_ORDER) 
        for code in BLOCK_ORDER:
            st.write("Trying code:", code)
            block_cap = BLOCK_UNIT_LIMITS.get(code)
            if block_cap is None:
                st.warning(f"Skipping code {code}: not found in BLOCK_UNIT_LIMITS")
                continue
...
with st.expander("📈 Visual Summary: Billing by Time Block (Before & After Optimization)", expanded=True):
    if st.session_state.get('billing_df') is not None:
        ...
        timeline = df.groupby(["Time Block", "Source"]).agg(
            Units=('Units', 'sum'),
            Total_Fee=('Total Fee', 'sum'),
            Calls=('Calls', 'sum')
        ).reset_index()

        # Set categorical order for time blocks
        timeline['Time Block'] = pd.Categorical(
            timeline['Time Block'], categories=CHRONOLOGICAL_BLOCK_ORDER, ordered=True
        )
        timeline = timeline.sort_values("Time Block")

        # Define custom colors: highlight Optimizer in gold/orange
        custom_colors = {
            "Original": "#4E79A7",    # blue
            "Optimizer": "#FFC300",   # bright gold/yellow
        }

        # Units bar: emphasize Optimizer
        fig_units = px.bar(
            timeline, x="Time Block", y="Units", color="Source",
            color_discrete_map=custom_colors,
            title="Units Billed per Time Block: Original vs. Optimized (Bright Highlight = Optimizer)",
            labels={"Units": "Units (15-min blocks)", "Source": "Entry Type"},
            text_auto=True,
            barmode="stack",
            hover_data=["Calls"]
        )
        fig_units.update_layout(legend=dict(orientation="h", y=-0.2))

        # Add fee annotation for each block (top of stack)
        fee_per_block = timeline.groupby("Time Block")["Total_Fee"].sum().reset_index()
        for i, row in fee_per_block.iterrows():
            fig_units.add_annotation(
                x=row["Time Block"],
                y=timeline[timeline["Time Block"] == row["Time Block"]]["Units"].sum(),
                text=f"${row['Total_Fee']:.0f}",
                showarrow=False,
                yshift=14,
        ...
        # Revenue bar: same structure, but y=Total_Fee
        fig_fees = px.bar(
            timeline, x="Time Block", y="Total_Fee", color="Source",
            color_discrete_map=custom_colors,
            title="Revenue per Time Block: Original vs. Optimized (Bright Highlight = Optimizer)",
            labels={"Total_Fee": "Fee ($)", "Source": "Entry Type"},
            text_auto=".2s",
            barmode="stack"
        )
        fig_fees.update_layout(legend=dict(orientation="h", y=-0.2))

        st.plotly_chart(fig_fees, use_container_width=True)

        # Show summary table per block (totals)
        block_summary = df.groupby("Time Block").agg(
            Units=('Units', 'sum'),
            Calls=('Calls', 'sum'),
            Total_Fee=('Total Fee', 'sum')
        ).reset_index()
        block_summary["Max Units"] = block_summary["Time Block"].map(BLOCK_UNIT_LIMITS)
        block_summary["Percent Used"] = (
            100 * block_summary["Units"] / block_summary["Max Units"]
        ).round(1).astype(str) + "%"

        st.markdown("#### ⏳ Block Summary Table")
        st.dataframe(block_summary, hide_index=True)

        # Key metrics
        st.markdown("### 💰 Billing Metrics")
        st.metric("Total Units", int(df["Units"].sum()))
        st.metric("Total Fee", f"${df['Total Fee'].sum():,.2f}")
        total_units_possible = sum(BLOCK_UNIT_LIMITS.values())
        actual_units_used = int(df["Units"].sum())
        efficiency_pct = 100.0 * actual_units_used / total_units_possible if total_units_possible else 0
        st.metric("Block Utilization Efficiency", f"{efficiency_pct:.1f}%")
        ...
        st.caption(
            "The bright bars show the revenue and unit gain from optimization/filler logic. "
            "Compare how many more units and dollars are captured per block!"
        )
    else:
        st.info("To see visual summaries, please run either the Calculate Billing or the Optimizer above.")
